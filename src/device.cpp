...
struct actuator DEVICE::getact(std::string n)
{
    ...
    {// if not found, print an error 
        char errorname[100];
        snprintf(errorname, sizeof errorname, "actuator's name does not exist : %s", n);
        perror(errorname);
...
{
    actuator a = {n, _chan, _vup, _vneutral, _vpush};
    return a;
}
